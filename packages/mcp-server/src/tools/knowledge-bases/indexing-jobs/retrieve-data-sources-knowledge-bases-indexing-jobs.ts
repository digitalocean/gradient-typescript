...
export const handler = async (client: Gradient, args: Record<string, unknown> | undefined) => {
  const { indexing_job_uuid, jq_filter, ...body } = args as any;
  return asTextContentResult(
    await maybeFilter(
      jq_filter,
      await client.knowledgeBases.indexingJobs.retrieveDataSources(indexing_job_uuid),
    ),
  );
};
