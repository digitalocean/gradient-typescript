...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'get',
  httpPath: '/v2/floating_ips/{floating_ip}/actions/{action_id}',
  operationId: 'floatingIPsAction_get',
};

export const tool: Tool = {
  name: 'retrieve_floating_ips_gpu_droplets_actions',
  description:
  ...
  inputSchema: {
    ...
    required: ['floating_ip', 'action_id'],
  },
  annotations: {
    readOnlyHint: true,
  },
...
export const handler = async (client: Gradient, args: Record<string, unknown> | undefined) => {
  const { action_id, jq_filter, ...body } = args as any;
  return asTextContentResult(
    await maybeFilter(jq_filter, await client.gpuDroplets.floatingIPs.actions.retrieve(action_id, body)),
  );
};
