...
describe('resource autoscale', () => {
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.gpuDroplets.autoscale.list({
    name: 'name',
    page: 1,
    per_page: 1,
  ...
  test.skip('deleteDangerous: only required params', async () => {
    const responsePromise = client.gpuDroplets.autoscale.deleteDangerous('0d3db13e-a604-4944-9827-7ec2642d32ac', { 'X-Dangerous': true });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('deleteDangerous: required and optional params', async () => {
    const response = await client.gpuDroplets.autoscale.deleteDangerous('0d3db13e-a604-4944-9827-7ec2642d32ac', { 'X-Dangerous': true });
  });
  ...
  test.skip('listHistory: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.gpuDroplets.autoscale.listHistory('0d3db13e-a604-4944-9827-7ec2642d32ac', { page: 1, per_page: 1 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Gradient.NotFoundError);
  });
  ...
  test.skip('listMembers: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.gpuDroplets.autoscale.listMembers('0d3db13e-a604-4944-9827-7ec2642d32ac', { page: 1, per_page: 1 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Gradient.NotFoundError);
  });
});
