...
describe('resource firewalls', () => {
  ...
  test.skip('create: request options and params are passed correctly', async () => {
    ...
    await expect(client.gpuDroplets.firewalls.create({ body: {
    droplet_ids: [8043964],
    inbound_rules: [{
    ports: '80',
    protocol: 'tcp',
  ...
  test.skip('update: only required params', async () => {
    const responsePromise = client.gpuDroplets.firewalls.update('bb4b2611-3d72-467b-8602-280330ecd65c', { firewall: { name: 'frontend-firewall' } });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('update: required and optional params', async () => {
    const response = await client.gpuDroplets.firewalls.update('bb4b2611-3d72-467b-8602-280330ecd65c', { firewall: {
    droplet_ids: [8043964],
    inbound_rules: [{
    ports: '8080',
    protocol: 'tcp',
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.gpuDroplets.firewalls.list({ page: 1, per_page: 1 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Gradient.NotFoundError);
  });
