...
describe('resource gpuDroplets', () => {
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.gpuDroplets.list({
    name: 'name',
    page: 1,
    per_page: 1,
    tag_name: 'tag_name',
  ...
  test.skip('listFirewalls: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.gpuDroplets.listFirewalls(3164444, { page: 1, per_page: 1 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Gradient.NotFoundError);
  });
  ...
  test.skip('listKernels: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.gpuDroplets.listKernels(3164444, { page: 1, per_page: 1 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Gradient.NotFoundError);
  });
  ...
  test.skip('listSnapshots: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.gpuDroplets.listSnapshots(3164444, { page: 1, per_page: 1 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Gradient.NotFoundError);
  });
});
