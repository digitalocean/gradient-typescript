...
describe('resource snapshots', () => {
  ...
  test.skip('retrieve: request options and params are passed correctly', async () => {
    ...
    await expect(client.nfs.snapshots.retrieve('0a1b2c3d-4e5f-6a7b-8c9d-0e1f2a3b4c5d', { region: 'region' }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.nfs.snapshots.list({ region: 'region', share_id: 'share_id' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Gradient.NotFoundError);
  });
  ...
  test.skip('delete: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.nfs.snapshots.delete('0a1b2c3d-4e5f-6a7b-8c9d-0e1f2a3b4c5d', { region: 'region' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Gradient.NotFoundError);
  });
});
