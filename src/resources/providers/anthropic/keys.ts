...
export class Keys extends APIResource {
  ...
  create(body: KeyCreateParams, options?: RequestOptions): APIPromise<KeyCreateResponse> {
    return this._client.post('/v2/genai/anthropic/keys', { body, ...options });
  }
  ...
  retrieve(apiKeyUuid: string, options?: RequestOptions): APIPromise<KeyRetrieveResponse> {
    return this._client.get(path`/v2/genai/anthropic/keys/${apiKeyUuid}`, options);
  }
  ...
  update(apiKeyUuid: string, body: KeyUpdateParams, options?: RequestOptions): APIPromise<KeyUpdateResponse> {
    return this._client.put(path`/v2/genai/anthropic/keys/${apiKeyUuid}`, { body, ...options });
  }
  ...
  list(query: KeyListParams | null | undefined = {}, options?: RequestOptions): APIPromise<KeyListResponse> {
    return this._client.get('/v2/genai/anthropic/keys', { query, ...options });
  }
  ...
  delete(apiKeyUuid: string, options?: RequestOptions): APIPromise<KeyDeleteResponse> {
    return this._client.delete(path`/v2/genai/anthropic/keys/${apiKeyUuid}`, options);
  }
  ...
  listAgents(
    ...
    return this._client.get(path`/v2/genai/anthropic/keys/${uuid}/agents`, { query, ...options });
  }
}
